...
title: 讀《The Harness Problem》後，我重新思考 AI coding 真正的瓶頸
...
現在談 AI coding，大家最常問的幾乎都是「哪個模型比較強」
...
作者認為，大家太習慣把 AI coding 的成敗歸因於 model，本質上像是在比模型排行榜
...
很多 AI coding 的失敗，不一定是 **理解失敗**，而可能是 **表達失敗**
...
大多數工程師沒辦法參與模型訓練，但卻可以參與 `tool_schemas`、`workflow`、`error_messages`、`state management` 這些設計

如果 harness 的改善真的能帶來接近 model upgrade 的收益，那就代表 AI coding 的進步，不一定只來自更大的模型，也來自更紮實的工具工程
...
如果過去大家一直在談 `Prompt engineering`，那接下來 AI coding 更值得投入的方向，可能會是 `Harness engineering`
